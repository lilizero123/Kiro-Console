...
export function CredentialCard({ credential }: CredentialCardProps) {
  ...
  const [usage, setUsage] = useState<BalanceResponse | null>(null)
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div className="col-span-2 space-y-1">
              ...
              <div className="h-2 rounded bg-muted overflow-hidden">
                <div
                  className={`h-full transition-all duration-300 ${
                    usage ? 'bg-green-500' : 'bg-muted-foreground/40'
                  }`}
                  style={{
                    width: `${
                      usage
                        ? Math.min(
                            usage.usageLimit > 0
                              ? (usage.remaining / usage.usageLimit) * 100
                              : 0,
                            100
                          )
                        : 0
              ...
              <div className="text-xs">
                {usage
                  ? `已用 ${usage.currentUsage.toFixed(2)} / ${usage.usageLimit.toFixed(2)}，剩余 ${usage.remaining.toFixed(2)}`
                  : usageError
