...
> **提示**：从 2026.1.5 起，程序首次运行会自动生成 `config.json`，其中除 API 相关密钥外的字段都已经写入推荐默认值。一般情况下无需手动编辑，直接运行并在 Admin UI 中设置管理员密钥与对外 API Key 即可。如果需要对监听地址、端口或代理做高级定制，可再编辑该文件。

创建 `config.json` 配置文件（可选，若不存在启动时会自动写入以下示例）：

```json
{
   "host": "127.0.0.1",   // 必配, 监听地址
   "port": 8990,  // 必配, 监听端口
   "apiKey": "sk-kiro-console-qazWSXedcRFV123456",  // 必配, 请求的鉴权 token
...
> 如果需要在启动时强制重置管理员登录密钥或对外 API Key，可在命令行增加 `--reset-admin-key` 或 `--reset-api-key` 参数，例如：
> ```
> cargo run --release -- --reset-admin-key --config config.json --credentials credentials.json
> ```
> 这样在访问 `http://127.0.0.1:PORT/admin` 时会再次进入初始化流程。
...
Kiro Console 提供自包含的 Docker 镜像，脚本可在干净 Ubuntu 上一条命令完成安装、构建与启动。
...
1. 安装缺失的 Docker / Git（Ubuntu）
2. 克隆指定分支源码并构建镜像（默认 `main`，镜像名 `kiro-console:latest`）
3. 在 `/var/lib/kiro-console` 准备 `config.json` 与 `credentials.json`
4. 以 `kiro-console` 容器名启动并映射 `8990` 端口

环境变量可覆盖关键配置：

| 变量 | 说明 | 默认值 |
|------|------|--------|
| `KIRO_CONSOLE_PORT` | 对外暴露端口 | `8990` |
| `KIRO_CONSOLE_IMAGE` | 构建后的镜像标签 | `kiro-console:latest` |
| `KIRO_CONSOLE_CONTAINER` | 容器名称 | `kiro-console` |
| `KIRO_CONSOLE_CONFIG_DIR` | 宿主机配置目录（挂载到 `/app/config`） | `/var/lib/kiro-console` |
| `KIRO_CONSOLE_REPO` | Git 仓库地址 | `https://github.com/lilizero123/Kiro-Console.git` |
| `KIRO_CONSOLE_BRANCH` | 拉取分支 | `main` |
...
首次启动后访问 `http://服务器IP:8990/admin` 设置管理员密钥及对外 API Key。
